...
public class GetJSONContent extends BlockingNativeCallableUnit {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(GetJSONContent.class);
    ...
    public void execute(Context context) {
        ...
        try {
            context.setReturnValues(JsonParser.parse(new String(messageContent,
                    StandardCharsets.UTF_8.name())));
        } catch (UnsupportedEncodingException exception) {
            LOGGER.error("Error while retrieving the json content of the message");
            RabbitMQUtils.returnError("Error while retrieving the json content of the message: "
